...
.dshncm-modebtn{flex:none;min-width:30px;height:30px;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-secondary);border-radius:8px;cursor:pointer;font-size:13px;padding:0 8px}
.dshncm-modebtn:hover{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary)}
.dshncm-modebtn[data-active=true]{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb,var(--dsw-alias-state-business-primary) 10%,transparent)}
...
`;
		...
		// ---- song / playlist rows ----
		function unplayableLabel(song) {
			if (song.visible !== false) return null;
			if (song.payPlayFlag || song.songFee > 0) return "付费";
			if (song.vipFlag || song.vipPlayFlag) return "VIP";
			return "无版权";
		}

		function unplayableTitle(song) {
			const label = unplayableLabel(song) || "不可播放";
			return label + " · 常见原因：无版权/已下架、VIP或付费专属、地区限制、需要登录、免费试听限制";
		}
		...
		function SongRow(props) {
			...
			return react.createElement(
				"div",
				{
					...
					title: blocked ? unplayableTitle(song) : (song.name + " - " + artistsOf(song)),
				},
				...
				react.createElement("div", { className: "dshncm-row-right" }, blocked ? unplayableLabel(song) : fmtMs(song.duration)),
		...
		function PlaylistRow(props) {
			...
			const onPlayAll = props.onPlayAll;
			const sub = (playlist.creatorNickName ? playlist.creatorNickName + " · " : "") + (playlist.trackCount != null ? playlist.trackCount + " 首" : "");
			return react.createElement(
				"div",
				{
					className: "dshncm-row",
					onClick: function () { onOpen(playlist); },
					role: "button",
					tabIndex: 0,
					onKeyDown: function (event) { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(playlist); } },
				...
				react.createElement("button", {
					type: "button",
					className: "dshncm-row-play",
					onClick: function (event) { event.stopPropagation(); if (onPlayAll) onPlayAll(playlist); },
					"aria-label": "播放整个歌单 " + playlist.name,
					title: "播放整个歌单",
				}, "▶"),
			);
		}
		...
		function MusicApp() {
			...
			// playback mode: order | loop | shuffle (persisted)
			const [playMode, setPlayMode] = useState(function () {
				try { return window.localStorage.getItem("dshncm:mode") || "order"; } catch (e) { return "order"; }
			});
				...
				function tick() {
					...
					api("/plugins/ncm/state").then(function (body) {
						...
						if (state) {
							...
							// loop mode: a natural end (playing -> stopped) replays the current song
							if (prevStatus === "playing" && state.status === "stopped"
								&& modeRef.current === "loop" && lastSongRef.current && !userInterruptRef.current) {
								const song = lastSongRef.current;
								userInterruptRef.current = false;
								setPlayer({
									status: "starting",
									title: song.name + (song.artists && song.artists.length ? " - " + artistsOf(song) : ""),
									position: 0,
									duration: song.duration ? song.duration / 1000 : undefined,
								});
								api("/plugins/ncm/play", {
									method: "POST",
			...
			function doPlay(song) {
				lastSongRef.current = { id: song.id, originalId: song.originalId, name: song.name, artists: song.artists, duration: song.duration };
				userInterruptRef.current = false;
			...
			function openPlaylist(playlist) {
				setPlaylistDetail({ playlist: playlist, tracks: null });
				run(function () {
					return api("/plugins/ncm/tracks?playlistId=" + encodeURIComponent(playlist.id) + "&limit=100")
						.then(function (body) {
							setPlaylistDetail({ playlist: playlist, tracks: body.records || [] });
						});
				});
			}
			...
			function doControl(action) {
				if (action === "stop" || action === "pause") userInterruptRef.current = true;
				return run(function () {
			...
			function cyclePlayMode() {
				setPlayMode(function (current) {
					const next = current === "order" ? "loop" : (current === "loop" ? "shuffle" : "order");
					try { window.localStorage.setItem("dshncm:mode", next); } catch (e) { }
					return next;
				});
			}

			// shuffle: pick a random playable song from the currently loaded lists
			function pickRandomSong() {
				const pools = [];
				if (Array.isArray(results)) pools.push.apply(pools, results);
				if (Array.isArray(daily)) pools.push.apply(pools, daily);
				if (favorite && Array.isArray(favorite.tracks)) pools.push.apply(pools, favorite.tracks);
				const playable = pools.filter(function (s) { return s && s.visible !== false && s.id; });
				if (playable.length === 0) {
					setError("随机模式需要先加载歌曲列表（搜索 / 每日推荐 / 我喜欢）");
					return;
				}
				doPlay(playable[Math.floor(Math.random() * playable.length)]);
			}

			function onNext() {
				if (modeRef.current === "shuffle") { pickRandomSong(); return; }
				doControl("next");
			}
			...
			function renderLoginBar() {
				// silent while logged in or before the first check completes
				if (loggedIn || login === null || login === undefined) return null;
				let text;
				if (login.pending) text = "登录中，请在打开的页面完成授权…";
				else if (login.error) text = "插件服务异常";
				else text = "未登录网易云";
				...
				return react.createElement("div", { className: "dshncm-login" }, children);
			}
			...
			function renderNowPlaying() {
				...
				children.push(react.createElement("div", { className: "dshncm-progress-time" },
					...
					react.createElement("button", { type: "button", className: "dshncm-iconbtn", onClick: onNext, disabled: busy, title: playMode === "shuffle" ? "下一首（随机）" : "下一首", "aria-label": "下一首" }, "⏭"),
					...
					react.createElement("button", {
						type: "button",
						className: "dshncm-modebtn",
						"data-active": playMode !== "order" || undefined,
						onClick: cyclePlayMode,
						title: playMode === "order" ? "顺序播放（播完即停）" : (playMode === "loop" ? "单曲循环" : "随机播放"),
						"aria-label": "播放模式",
			...
			function renderPlaylistsTab() {
				...
				if (playlistDetail) {
					...
					const children = [];
					children.push(react.createElement("div", { className: "dshncm-search" },
						react.createElement("button", { type: "button", className: "dshncm-btn", onClick: closePlaylist }, "← 返回歌单"),
						react.createElement("button", {
							type: "button",
							className: "dshncm-btn",
							"data-primary": true,
							onClick: function () { doPlaylist(playlist); },
							disabled: busy,
							title: "顺序播放整个歌单",
						}, "▶ 播放全部"),
					));
					children.push(react.createElement("div", { className: "dshncm-hint", style: { padding: "4px 0 8px", fontSize: "12px" } },
						"📃 " + playlist.name + (playlist.trackCount != null ? "（" + playlist.trackCount + " 首）" : "")));
					if (playlistDetail.tracks === null) {
						children.push(react.createElement("div", { className: "dshncm-hint" }, "加载歌曲…"));
					} else if (playlistDetail.tracks.length === 0) {
						children.push(react.createElement("div", { className: "dshncm-hint" }, "歌单为空"));
					} else {
						const rows = playlistDetail.tracks.map(function (song) {
							return react.createElement(SongRow, { key: song.id || song.originalId, song: song, onPlay: doPlay });
						});
						children.push(react.createElement("div", { className: "dshncm-list" }, rows));
					}
				...
				for (let i = 0; i < created.length; i++) {
					rows.push(react.createElement(PlaylistRow, { key: created[i].id || created[i].originalId, playlist: created[i], onOpen: openPlaylist, onPlayAll: doPlaylist }));
				}
				for (let i = 0; i < collected.length; i++) {
					rows.push(react.createElement(PlaylistRow, { key: "c-" + (collected[i].id || collected[i].originalId), playlist: collected[i], onOpen: openPlaylist, onPlayAll: doPlaylist }));
				}
				return react.createElement("div", { className: "dshncm-list" }, rows);
			}
